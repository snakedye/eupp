...
///
/// Use [`LedgerView::as_ledger`] to conditionally access ledger-specific
/// capabilities (e.g. fetching full blocks) when the `Full` variant is active.
...
impl<I, L: Ledger> LedgerView for NodeStore<I, L> {
    type Ledger<'a>
        = L
    where
        Self: 'a;
